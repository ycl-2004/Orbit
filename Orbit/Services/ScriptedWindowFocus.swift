...
enum ScriptedWindowFocus {
    ...
    /// The Automation permission is granted per target app, and macOS only asks
    /// on the first Apple Event sent to each one — which, left alone, means one
    /// surprise dialog per app in the middle of a switch. This asks for all of
    /// them up front instead, from Settings, where the dialogs have context.
    ...
    /// real windows, yet Accessibility reports none — the Chromium signature.
    /// Anything else never receives an Apple Event, so prompting for it would
    /// be pure noise.
    static func needsAppleEventsFallback(hasRealWindows: Bool, axWindowCount: Int) -> Bool {
        hasRealWindows && axWindowCount == 0
    }

    /// Walks the running apps that will need the Apple Events fallback and asks
    /// macOS to show each missing Automation prompt now.
    ///
    /// Runs on a background queue: `AEDeterminePermissionToAutomateTarget`
    /// blocks for as long as its dialog is on screen, and the dialogs appear
    /// one after another.
    ///
    ...
    static func preauthorizeRunningApps(
        ...
        let candidates: [(String, String)] = NSWorkspace.shared.runningApplications
            ...
            .compactMap { app in
                ...
                let axCount = (value as? [AXUIElement])?.count ?? 0
                guard needsAppleEventsFallback(
    ...
    private static func requestAutomationConsent(bundleIdentifier: String) -> AutomationConsent {
        ...
        let created = data.withUnsafeBytes { bytes in
            AECreateDesc(typeApplicationBundleID, bytes.baseAddress, data.count, &target)
        }
