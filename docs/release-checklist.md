...
```bash
bash scripts/package_release.sh 1.6.0
...
GitHub Actions runs the metadata check, a Debug Universal 2 build, and the 64
`OrbitTests` tests for every push and pull request. A version tag must match the
Xcode `MARKETING_VERSION`, for example `v1.6.0`.
