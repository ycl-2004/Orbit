...
当前 Xcode 工程版本为 `1.6.0（build 7）`。从 `v1.5.1` 以来的改动见
[1.6.0 发布说明](CHANGELOG.md)。

## 为什么用 Orbit

- **固定的第一目标。** 当前应用有可切换的其他窗口时，最近的另一扇窗口入口固定在 12 点钟方向；否则这里才是上一个应用。卡片数量或预览设置变化时，第一个键盘目标不会移动。
...
- 支持调整界面语言、最近使用/按名称排列、从取消开始/快速切换、预览大小（70%–150%）、触发键、取消选择键、字母/数字快捷键、长按阈值、环出现位置、卡片大小/材质和开机启动。
...
- 默认使用方向键（以及 Tab）导航；环处于未选择状态时，任一方向键都会先进入 12 点目标：当前应用有其他窗口时是最近的另一扇窗口，否则是上一个应用。继续按键则按最近使用顺序前进。可在设置中启用数字键 `1`–`9` 或首字母匹配作为额外快捷键。
...
权限可以在**系统设置 → 隐私与安全性 → 辅助功能 / 屏幕录制 / 自动化**中撤销。
...
和 pull request 上执行元数据检查、Debug 构建以及 64 个 `OrbitTests` 测试。
...
- 当前源码版本：`1.6.0（build 7）`。
- App 默认使用英文界面；应用内语言选择器提供 English、简体中文、繁體中文、日本語、한국어、Deutsch、Français、Русский、Dansk、Norsk bokmål 和 Esperanto。

## 致谢

Orbit 的核心构想 —— 按住修饰键，正在运行的应用便围着光标排成一圈 —— 受
[yuzeguitarist/Orbit](https://github.com/yuzeguitarist/Orbit)（作者 Yuze Pan，
潘禹泽）启发。我在 2026 年看到那个项目，很喜欢它的设计理念，想做一个更贴合
自己使用习惯的版本。

动手之前我联系了原作者。对方明确表示代码并不完全开源、不可直接修改使用，同时
也明确欢迎我独立开发一个类似的作品。本项目正是在这个前提下从零写成的：共享的
是理念，不是源码。感谢 Yuze Pan 的创意，也感谢他慷慨的答复。
